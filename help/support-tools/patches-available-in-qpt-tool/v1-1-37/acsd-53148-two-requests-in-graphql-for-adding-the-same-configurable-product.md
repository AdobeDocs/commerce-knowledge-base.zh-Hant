...
ACSD-53148修補程式修正了GraphQL中將相同可設定產品新增至購物車的兩個平行請求，在購物車上會產生兩個具有相同產品SKU的單獨專案的問題。 此修補程式適用於 [[!DNL Quality Patches Tool (QPT)]](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 已安裝1.1.37。 修補程式ID為ACSD-53148。 請注意，此問題已排程在Adobe Commerce 2.4.7中修正。
...
1. 使用SKU建立可設定的產品 *測試* 以及簡單的SKU產品 *測試 — A*.
1. 透過GraphQL建立新的空白購物車（使用您自己的變更位置） [!DNL URL])：
...
1. 執行兩個並行請求，將相同的產品新增至購物車（變更） *cartID* 和位置)：
