...
MDVA-40399修補程式修正了無法透過Rest API同時建立相同訂單之部分發票的問題。 此修補程式適用於 [品質修補工具(QPT)](https://devdocs.magento.com/guides/v2.4/comp-mgr/patching.html#mqp) 已安裝1.1.4。 修補程式ID為MDVA-40399。 請注意，此問題已排程在Adobe Commerce 2.4.4中修正。
...
* 兩個商業發票都必須成功建立。
* `qty_invoiced` 應針對以下兩張商業發票更新： `sales_order_item` 表格。
* 這兩種產品都應該有已開立商業發票的數量。

<u>實際結果</u>：

* 兩個商業發票都已成功建立。
* `qty_invoiced` 不會針對下列其中一個商業發票進行更新： `sales_order_item` 表格。
* 在管理員的 **訂單檢視** 頁面，只針對一個產品顯示商業發票數量。
