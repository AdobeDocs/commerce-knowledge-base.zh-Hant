...
MDVA-42768修補程式修正了當子產品（可設定的產品）無存貨時，GraphQL顯示錯誤價格的問題。 此修補程式適用於 [品質修補工具(QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 已安裝1.1.10。 修補程式ID為MDVA-42768。 請注意，此問題已排程在Adobe Commerce 2.4.5中修正。
...
1. 啟用「Commerce管理員」中的「顯示缺貨產品」設定，方法是前往 **商店** > **設定** > **目錄** > **詳細目錄**.
1. 建立可設定的產品並為其指派簡單的子產品。
1. 將變體（簡單）產品的庫存設定為 **無庫存**.
1. 重新索引。
1. 執行下列GraphQL查詢：
