...
MDVA-43601修補程式修正了從中移除觸發程式的問題 `catalogrule_product_price` 完整重新索引後的資料表 `catalogrule_rule` 或 `catalogrule_product`. 此修補程式適用於 [品質修補工具(QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 已安裝1.1.13。 修補程式ID為MDVA-43601。 請注意，此問題已排程在Adobe Commerce 2.4.5中修正。
...
觸發器移除自 `catalogrule_product_price` 完整重新索引後的資料表 `catalogrule_rule` 或 `catalogrule_product`.

<u>要再現的步驟</u>：

1. 將所有索引子設定為 *依排程更新*.
1. 檢查已建立的觸發器 `catalogrule_product_price` 執行下列SQL要求以建立表格：

   ```sql
   show triggers like '%catalogrule_%'\G
   ```

1. 手動重新索引 `catalogrule_rule` 或 `catalogrule_product` 藉由執行下列命令： `bin/magento indexer:reindex catalogrule_rule`
1. 檢查的觸發程式 `catalogrule_product_price` 表格重新整理。

<u>預期結果</u>：

觸發於 `catalogrule_product_price` 資料表會在完整重新索引後保留。

<u>實際結果</u>：

找不到觸發程式 `catalogrule_product_price` 表格。
