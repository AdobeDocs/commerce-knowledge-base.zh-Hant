...
MDVA-38559修補程式修正了 `/V1/customers/search` 對於有多個訂閱的客戶，API會傳回錯誤。 此修補程式適用於 [品質修補工具(QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 已安裝1.1.15。 修補程式ID為MDVA-38559。 請注意，問題已在Adobe Commerce 2.4.3中修正。
...
`/V1/customers/search` API會為具有多個訂閱的客戶傳回錯誤。
...
1. 前往 **儲存** > **設定** > **客戶** > **客戶組態** > **帳戶共用選項** 並選取 **全域**.
1. 前往 **客戶** > **所有客戶**，選取 **編輯** ，然後選取「 」 **電子報**.
1. 訂閱一個以上網站的電子報並儲存客戶。
1. 傳送下列請求：
...
下列錯誤會記錄到exception.log： *具有相同ID「1」的專案(Magento\Customer\Model\Customer\Interceptor)已經存在。*
