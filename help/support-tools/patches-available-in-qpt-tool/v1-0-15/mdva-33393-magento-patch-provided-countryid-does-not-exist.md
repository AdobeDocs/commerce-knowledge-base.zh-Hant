...
MDVA-33393修補程式解決錯誤訊息： *提供的國家/地區ID不存在的問題。*&#x200B;會顯示在例外狀況記錄檔中。

安裝[品質修補工具(QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 1.0.15時，即可使用此修補程式。 請注意，此問題已排程在Adobe Commerce 2.4.3中修正。
...
使用者看到錯誤訊息： *提供的國家/地區ID不存在*&#x200B;在例外狀況記錄中。 在不同的情況（包括使用者選取送貨方法時）下，可能會擲回錯誤。 當有許多國家/地區缺少郵遞區號驗證時，就會發生這種情況。 這表示郵遞區號驗證設定檔不完整。
