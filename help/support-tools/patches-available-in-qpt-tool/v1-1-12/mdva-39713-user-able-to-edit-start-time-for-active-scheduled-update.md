...
MDVA-39713修補程式修正使用者可編輯作用中排程更新之開始時間的問題。 此修補程式適用於 [品質修補工具(QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 已安裝1.1.12。 修補程式ID為MDVA-39713。 請注意，此問題已排程在Adobe Commerce 2.4.5中修正。
...
1. 建立新的CMS頁面。
1. 選取 **排程新更新** 並設定 **開始日期** 至目前的+1分鐘。
1. 執行命令以啟動排定的更新 `bin/magento cron:run --group=staging` 在本機環境中。 請等候數分鐘，然後檢查排程是否作用中。
1. 排程啟動後，請重新整理頁面。
1. 按一下 **檢視/編輯** 在排程變更區段中。
1. 新增+2分鐘以編輯時間並儲存變更。
1. 儲存CMS頁面。
...
使用者收到類似以下的錯誤 *具有相同ID 「11」的專案(Magento\Cms\Model\Page)已經存在。*
