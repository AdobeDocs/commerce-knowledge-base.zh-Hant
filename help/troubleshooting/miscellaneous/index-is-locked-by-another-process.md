...
在您的CLI中完整重新索引期間，Adobe Commerce會提供錯誤訊息： *&#39;Index被另一個重新索引程式鎖定。 正在跳過。&#39;*&#x200B;換句話說，當處理程式或索引型別鎖定時，您將無法重新索引該特定鎖定的索引型別。 重新索引將一律跳過該索引型別。
...
1. 例如，假設`cataloginventory_stock`索引型別已鎖定。
1. 當您嘗試執行CLI命令來重新索引所有資料時：
...
1. 如上所示，已略過`cataloginventory_stock`索引程式。
