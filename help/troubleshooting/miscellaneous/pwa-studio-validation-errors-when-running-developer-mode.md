...
description: 本主題討論在適用於Adobe Commerce的Progressive Web App (PWA) Studio中執行開發人員模式時，因先前未建立venia-concept （Venia是PWA店面）環境檔案而發生驗證錯誤的解決方案。 此檔案會保留您本機開發環境的變數。
...
本主題討論在適用於Adobe Commerce的Progressive Web App (PWA) Studio中執行開發人員模式時，因先前未建立venia-concept （Venia是PWA店面）環境檔案而發生驗證錯誤的解決方案。 此檔案會保留您本機開發環境的變數。
...
* 在適用於Adobe Commerce的PWA Studio中執行開發人員模式。
