...
1. 取得MySQL資料庫認證： `echo $MAGENTO_CLOUD_RELATIONSHIPS | base64 -d | json_pp`
1. 使用`mysql`連線到資料庫： `mysql -hdatabase.internal -uuser -ppassword main`
1. 選取`main`資料庫： `use main`
1. 尋找所有正在執行的[!DNL cron]工作： `SELECT * FROM cron_schedule WHERE status = 'running';`
1. 複製任何執行時間超過平常的工作的`job_code`。
1. 使用先前步驟的排程ID來解除鎖定特定[!DNL cron]工作： `./vendor/bin/ece-tools cron:unlock --job-code=<job_code_1> [... --job-code=<job_code_x>]`
